...
const MovieList = () => {
  ...
  const checkScrollPosition = () => {
    if (sliderRef.current) {
      ...
      setIsAtEnd(scrollLeft + clientWidth >= scrollWidth - marginError); // Kondisi akhir dengan toleransi
    }
  };
  ...
  return (
    <div className="relative px-6 py-4 lg:px-20">
      <h1 className="mb-5 text-xl font-medium">Trending</h1>
      {/* movie section */}
      {!isAtStart && ( // Tampilkan tombol jika tidak di posisi awal
        <button
          onClick={scrollLeft}
          className="slide-button absolute left-2 top-1/2 z-10 flex h-12 w-8 items-center justify-center rounded border bg-primary opacity-75 transition hover:opacity-100 lg:left-16"
        >
          <svg
      ...
      {!isAtEnd && ( // Tampilkan tombol jika tidak di posisi akhir
        <button
          onClick={scrollRight}
          className="slide-button absolute right-2 top-1/2 z-10 flex h-12 w-8 items-center justify-center rounded border bg-primary opacity-75 transition hover:opacity-100 lg:right-16"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
      ...
      <div className="slider-scrollbar group flex h-6 w-full items-center">
        <div className="scrollbar-track relative h-0.5 w-full rounded bg-primary group-hover:h-1">
          <div className="scrollbar-thumb absolute h-full w-1/2 cursor-grab border-inherit bg-secondary after:absolute after:-bottom-2.5 after:-top-2.5 after:left-0 after:right-0 active:-top-0.5 active:h-1.5 active:cursor-grabbing"></div>
        </div>
      </div>
      {/* End movie section */}
    </div>
